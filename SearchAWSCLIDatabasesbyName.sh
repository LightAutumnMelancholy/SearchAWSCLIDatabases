...
function CheckRequirements() {
        if [ -z "$databaseForQuery" ] || [ -z "$profileName"  ] || [ -z "$awsCliLocale" ] ; then
        	...
        	profileDefined=$(sed -e s'/\[//g;s/\]//g' "$credsFile" | grep -c -m 1 "$profileName")
...
function CheckDatabase() {
        ...
        for i in $AWSREGIONS
            do 
                    ...
                    if ! aws rds describe-db-instances --profile "$profileName" --region "$i" --output $outputType 2> /dev/null \
                            | grep --color -m "$matchCount" -A "$lineCount" -i  -E DBInstanceIdentifier.*"${databaseForQuery}" 2> /dev/null ; then
                            printf "%s\n" "[INFO]: Database not found in $i region."
                    else
                            echo  -e "\033[33;5;7m[SUCCESS]: DATABASE OR DATABASE(S) FOUND\033[0m"
                        exit 0
                    fi
            done
}
